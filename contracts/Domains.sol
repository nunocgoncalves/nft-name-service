...
contract Domains is ERC721URIStorage {
  ...
  function setRecord(string calldata name, string calldata record) public {
    // If the message sender does not own the domain revert transaction (Unauthorized)
  ...
  function price(string calldata name) public pure returns(uint) {
    ...
    require(len > 1);
    if (len == 2) {
        return 5 * 10**14; // 0.005 MATIC = 5 000 000 000 000 000 (18 decimals)
    } else if (len <= 4) {
        return 3 * 10**14; // 0.003 MATIC = 5 000 000 000 000 000 (18 decimals)
    } else {
        return 1 * 10**14; // 0.001 MATIC = 5 000 000 000 000 000 (18 decimals)
    }
  }
  ...
  function register(string calldata name) public payable {
    ...
    // If the domain already exists revert transaction
    if (domains[name] != address(0)) revert AlreadyRegistered();
    // If the domain name is invalid (too short or too long) revert the transaction
    if (!valid(name)) revert InvalidName(name);
    
    // Check for transaction payment
  ...
  console.log("Getting all minted domains from contract");
  string[] memory allNames = new string[](_tokenIds.current());
  for (uint i = 0; i < _tokenIds.current(); i++) {
    allNames[i] = names[i];
    console.log("Domain for token %d is %s", i, allNames[i]);
  }
